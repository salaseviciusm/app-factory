...
| 4 | 2026-08-02 | Bring-up: OpenClaw installed/onboarded with Claude Max auth, workspace verified (agent answers as Chief of Staff). Three doc drifts found and fixed by testing against the live tool: `$env` SecretRef → canonical form; symlinked skills need `skills.load.allowSymlinkTargets` (→ D14); seeded `BOOTSTRAP.md` overrides the factory persona until deleted. Auth choice made founder-style: cheapest viable (subscription) with recorded upgrade trigger (API key if rate limits bite). Lesson reinforced: runbooks earn their truth by being executed; verify against the installed version, not the docs from memory. |
